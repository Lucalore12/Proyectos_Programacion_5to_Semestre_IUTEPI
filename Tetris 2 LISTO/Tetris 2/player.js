class Player
{
    ...
    drop() {
        ...
        if (this.arena.collide(this)) {
            ...
            if (this.arena.collide(this)) { // game over
                ...
                this.saveScore(); // Guarda el puntaje al final del juego
                this.score = 0;
                this.level = 1;
                this.dropInterval = 1;
                alert('Game Over! Tu puntaje fue: ' + this.score);
                // Aquí podrías también mostrar los mejores puntajes
                this.showScores();
            }
        }
    }
    ...
    showScores() {
        const scoreList = this.scores.map((score, index) => `${index + 1}. ${score.name}: ${score.score}`).join('\n');
        alert('Mejores Puntajes:\n' + scoreList);
    }
    update(time = 0) {
